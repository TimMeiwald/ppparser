...
impl<T: Cache, S: Publisher> Context<T, S> {
    pub fn new(size_of_source: u32, number_of_structs: u32) -> Self {
        ...
        let stack: RefCell<S> = RefCell::new(S::new(size_of_source as usize, number_of_structs as usize));
        let phantom = PhantomData::<S>;
        let root_node = stack.borrow_mut().create_node(Rules::Grammar, 0, size_of_source, None, true);
