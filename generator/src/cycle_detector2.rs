...
impl<'a> GetInvolvedSets<'a> {
    fn new(
        ...
        slf.cleanup_involved_sets();
        slf.involved_sets
        // For each rules involved sets, check involved sets of other rules and remove them from involved set.
    }

    fn cleanup_involved_sets(&mut self) {
        // Rules are hierarchical so the involved set of a rule that calls another LR rule
        // includes it's involved set but this is not wanted behaviour. So we look up each rule in an involved set
        // and remove it's involved rules from the set of the current rule.
        let mut new_involved_sets: HashMap<String, HashSet<String>> = HashMap::new();
        for (parent_rule_name, involved_set) in &self.involved_sets {
            let mut fresh_involved_set: HashSet<String> = involved_set.clone();
            println!("Rule Name: {parent_rule_name}");
            println!("Before fresh Involved Set: {fresh_involved_set:?}");
            for rule_name in involved_set {
                let other_involved_set = self.involved_sets.get(rule_name).expect("Should exist");
                println!("Other Involved Set: {other_involved_set:?}");
                // Remove all rules that are contained in any other involved set of the rules in the involved set.
                involved_set
                    .iter()
                    .filter(|&e| other_involved_set.contains(e))
                    .for_each(|rule_name: &String| {
                        println!("Removing: {rule_name}");
                        fresh_involved_set.remove(rule_name);
                    });
            }
            println!("After fresh Involved Set: {fresh_involved_set:?}");
            new_involved_sets.insert(parent_rule_name.to_string(), fresh_involved_set);
        }
        self.involved_sets = new_involved_sets;
    }
    ...
    fn get_involved_set(&self, rule_name: &String) -> HashSet<String> {
        ...
        let involved_set = involved_set
            ...
            .filter(|r| {
                ...
                match s {
                    None => {
                        panic!("Rule should exist in is_left_recursive_rule")
                    }
                    Some(s) => match s {
