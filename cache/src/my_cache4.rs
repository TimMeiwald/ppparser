...
impl<T: Copy> Cache<T> for MyCache4<T> {
    ...
    fn push(&mut self, rule: Rules, is_true: bool, start_position: u32, end_position: u32, stack_index: Key<T> ) {
    ...
    fn check(&self, rule: Rules, start_position: u32) -> Option<(bool, u32, Key<T>)> {
        ...
        let indexed: Key<T> = self.indexes[index];
        if end_position != 0 {
