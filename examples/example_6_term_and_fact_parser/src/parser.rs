...
pub fn term<T: Context>(
    ...
    let closure_1 =
        _var_name_indirect_left_recursion(user_state, &involved_set, Rules::Term, context, term);
    let closure_2 = _terminal(b'+');
    let closure_3 = _sequence(&closure_1, &closure_2);
    let involved_set: Vec<Rules> = [Rules::Fact].to_vec();
    let closure_4 =
        _var_name_indirect_left_recursion(user_state, &involved_set, Rules::Fact, context, fact);
