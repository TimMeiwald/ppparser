...
pub struct UnsafeTree {
    last_node: Option<Key>,
    nodes: Vec<Node>,
}

impl Publisher for UnsafeTree {
    ...
    fn set_node_start_position(&mut self, index: Key, start_position: u32) {
        unsafe {
            let node = self.nodes.get_unchecked_mut(usize::from(index));
            let node = node as *mut Node;
            (*node).start_position = start_position;
        }
    ...
    fn set_node_end_position(&mut self, index: Key, end_position: u32) {
        unsafe {
            let node = self.nodes.get_unchecked_mut(usize::from(index));
            let node = node as *mut Node;
            (*node).end_position = end_position;
        }
    }
...
impl UnsafeTree {
    fn get_node(&self, index: Key) -> &Node {
        unsafe { self.nodes.get_unchecked(usize::from(index)) }
    }

    fn get_mut_node(&mut self, index: Key) -> &mut Node {
        unsafe { self.nodes.get_unchecked_mut(usize::from(index)) }
    }
